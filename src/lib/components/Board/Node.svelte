<script lang="ts">
  ...
  function onPanDown(event: GestureCustomEvent) {
    ...
    if (srcEvent.button === MOUSE_BUTTON_LEFT) {
      ...
      dragging = true;
      globalOffset = [srcEvent.clientX - globalPosition[0], srcEvent.clientY - globalPosition[1]];
    }
  }
  ...
  function onPanMove(event: GestureCustomEvent) {
    if (dragging) {
      ...
      const offset = [srcEvent.clientX - globalOffset[0], srcEvent.clientY - globalOffset[1]];

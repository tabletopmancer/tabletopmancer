...
<div
  ...
  aria-label="Settings"
>
  <div class="flex h-[28rem] w-full max-w-2xl overflow-hidden rounded-xl bg-gray-900 shadow-xl">
    <nav class="flex w-40 shrink-0 flex-col gap-1 border-r border-white/10 bg-black/20 p-3">
      <h2 class="mb-2 px-2 text-sm font-semibold text-gray-100">Settings</h2>
      {#each TABS as t (t.id)}
        <button
          onclick={() => (tab = t.id)}
        ...
        {#if tab === "general"}
          ...
          >
            <span class="flex flex-col">
              <span class="text-sm font-semibold text-gray-100">Online</span>
              ...
              {:else}
                <ul class="space-y-1">
                  {#each players as player (player.id)}
                    ...
                    >
                      <span class="truncate text-gray-100">{player.name}</span>
